...
/**
 * Design-system provider for the embedded preview documents.
 *
 * Unlike the nuqs-backed `DesignSystemProvider` used by the components
 * workbench, this one reads its params straight off `window.location.search`
 * on its FIRST client render, so a framed example paints in the right style
 * immediately instead of waiting for a postMessage round trip, and it needs no
 * Suspense boundary. Live customizer changes still arrive by postMessage.
 *
 * The keys it accepts are the `Pick` list above; keep them in step with
 * CATALOG_FRAME_DESIGN_KEYS in lib/component-preview-frame.ts.
 */
export function PreviewDesignSystemProvider({
  ...
  React.useEffect(() => {
    ...
    // Let the embedded preview scroll when its content is taller than the
    // iframe. This was "hidden", which clipped a long example with no
    // scrollbar.
    //
    ...
    // to its content height, so there is nothing left for it to scroll.
    // Instead we pin the document to the iframe viewport and make the BODY the
    // scroll container: html clips at 100% height, body is exactly one viewport
    // tall (100svh) with `overflow-y: auto`, so a taller example gets a real
    // scrollbar while one that fits shows none. `overscroll-behavior: none`
    // keeps the scroll contained to the iframe (no chaining to the host page),
    // and `PreviewScrollForwarder` hands the gesture back to the host once
  ...
  // its content, but no consumer ever subscribed: the host authors an
  // explicit frame height instead (see
  // `resolveComponentPreviewFrameHeight` in
  // lib/component-preview-frame.ts). With many preview iframes on one
  // listing page, that effect ran inside every child frame on every
  // parent resize event, forcing N x multiple synchronous reflows per
  // frame and visibly janking window resize.
  //
