...
import { LockButton } from "@/app/(create)/design-system/lock-button"
import {
  Picker,
  PickerContent,
  PickerGroup,
...
const LazyIconLibraryPreview = dynamic(
  () =>
    import("./icon-library-preview").then((mod) => ({
      default: mod.IconLibraryPreview,
    })),
  {
    ssr: false,
    loading: () => (
...
export function IconLibraryPicker({
  ...
  const currentIconLibrary = React.useMemo(
    () =>
      iconLibraries[
        (params.iconLibrary ?? config.iconLibrary) as keyof typeof iconLibraries
      ],
    [params.iconLibrary, config.iconLibrary]
  )

  const handleValueChange = React.useCallback(
    (value: string) => {
      const newIconLibrary = value as IconLibraryName
      setParams({ iconLibrary: newIconLibrary })
      setConfig((prev) => ({ ...prev, iconLibrary: newIconLibrary }))
      // Notify any IconPlaceholder consumers that read iconLibrary via
      // the storage-backed `useIconLibrary` hook — same-tab updates
      // need an explicit signal since the native `storage` event only
      // fires across tabs.
    ...
    <div className="group/picker relative">
      <Picker open={open} onOpenChange={setOpen}>
        <PickerTrigger>
          ...
          <div className="text-site-foreground *:[svg]:text-site-foreground! pointer-events-none absolute top-1/2 right-2 flex size-4 -translate-y-1/2 items-center justify-center text-base select-none">
    ...
    >
      <Item size="xs">
        <ItemContent className="gap-1">
          ...
          {previewEnabled ? (
            <LazyIconLibraryPreview iconLibrary={iconLibrary.name} />
          ) : (
            <div className="-mx-1 grid w-full grid-cols-7 gap-2">
              {Array.from({ length: 14 }).map((_, index) => (
                <div
                  key={index}
                  className="bg-site-muted site-rounded-sm size-6"
                />
              ))}
            </div>
          )}
