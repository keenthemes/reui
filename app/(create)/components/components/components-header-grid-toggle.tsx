...
interface ComponentsHeaderGridToggleProps {
  className?: string
}

export function ComponentsHeaderGridToggle({
  className,
}: ComponentsHeaderGridToggleProps) {
  ...
  const handleValueChange = (val: string) => {
    ...
    const numVal = Number(val) as ComponentGridMode
    setConfig({
      ...config,
      gridColumns: numVal,
    })
  ...
  return (
    ...
    >
      <TabsList className="h-8 p-0.5">
        {/* Component listing: 1 (row) or 2 columns */}
        <TabsTrigger
          value="1"
