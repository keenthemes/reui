...
function emitComponentsSearchChange(search: string | null) {
...
interface ComponentsHeaderSearchProps {
  placeholder?: string
  className?: string
}

export function ComponentsHeaderSearch({
  placeholder = "Search components...",
  className,
}: ComponentsHeaderSearchProps) {
  ...
  const getSearchValueToCommit = useCallback((value: string) => {
    const normalizedValue = normalizeComponentSearchQuery(value)
    return hasActiveComponentSearch(normalizedValue) ? normalizedValue : null
  }, [])
  ...
  const commitSearch = useCallback(
    (value: string) => {
      ...
      if (isOnCategoryPage) {
        ...
        emitComponentsSearchChange(finalValue)
        return
      }
  ...
  const handleClear = () => {
    ...
    if (isOnCategoryPage) {
      ...
      emitComponentsSearchChange(null)
    } else {
