...
function EventCalendarEvent<TData = unknown>({
  ...
  const chip = (
    <Comp
      ...
      // merged, not spread: every tint, ring and dot reads --ec-event-color,
      // so a consumer style prop must not replace it (base uses mergeProps)
      style={
        {
      ...
      // mergeProps gives base, so neither side loses its listener.
