---
title: Data Grid
description: A powerful TanStack Table v9 data grid with sorting, filtering, pagination, footer rows, drag-and-drop, row and column virtualization, infinite scroll, row pinning, and tree rows.
component: true
base: radix
links:
...
- `sortFns: { basic: sortFn_basic, text: sortFn_text }`. On v9 a string `sortFn` resolves against this registry
  alone, so `"basic"` and `"text"` (plus the default `"auto"`) are the only names this bundle types. Any other
  built-in (`"alphanumeric"`, `"datetime"`, `"textCaseSensitive"`) has to be registered in your own bundle or
  passed as a function. `"auto"` keeps working, but when it infers an unregistered name it warns in development
  and falls back.
...
<Callout title="Upgrading from v8" variant="warning">

These are the changes that bite hardest when porting existing tables:

- **Every generic gained a leading `TFeatures`.** `ColumnDef<TData, TValue>` becomes
  `ColumnDef<DataGridFeatures, TData, TValue>`, and `Row`, `Column`, `Cell`, `Header` and `Table` shift the same
  way. This is the first wall of compile errors in any ported file.
- **`useReactTable` is now `useTable`, and the `get*RowModel` options are gone.** Row models moved into the
  `features` bundle (`sortedRowModel`, `filteredRowModel`, `paginatedRowModel` and friends), which is what
  `dataGridFeatures` hands you.
- **A registered `paginatedRowModel` always slices.** It is the one row model that is not inert, so a grid that
  used to render every row now shows only `pageSize` rows (10 by default). When the data is already one page, or
  the grid renders all rows, set `manualPagination: true`.
- **Column pinning moved from `left`/`right` to `start`/`end`**, across `ColumnPinningState`, `column.pin()`,
  `getIsPinned()` and the `getLeft*`/`getRight*` method families. The grid's `data-pinned`, `data-last-col` and
  `data-outer-pinned-col` attributes now emit `start` / `end` too, so CSS targeting `[data-pinned="left"]` must
  be updated.
- **`getIsSomeRowsSelected()` and `getIsSomePageRowsSelected()` changed meaning**, from "some but not all" to
  "at least one". An indeterminate checkbox that does not also exclude the all-selected case will stay
  indeterminate forever. Guard it: `getIsSomePageRowsSelected() && !getIsAllPageRowsSelected()`.
- **State reads and names moved.** `table.getState()` is gone and reads go through `table.state`, the
  `columnSizingInfo` slice is now `columnResizing` (`setColumnSizingInfo` is `setColumnResizing`),
  `VisibilityState` is `ColumnVisibilityState`, and `sortingFn` / `SortingFn` are `sortFn` / `SortFn`.
- **State shapes got stricter.** `ColumnPinningState` requires both `start` and `end`, `RowPinningState` requires
  both `top` and `bottom`, and `RowSelectionState` narrowed to `Record<string, true>`, so deselecting by writing
  `false` no longer type-checks.
...
aggregates, row and column virtualization, controlled row and column scrolling,
infinite scroll, and row pinning) browse the
...
A virtualized table renderer using `@tanstack/react-virtual` for row virtualization, optional center-column virtualization, infinite scroll, footer rows, and pinned rows when `rowsPinnable` is enabled. Row virtualization remains enabled by default and can be customized with `virtualizerOptions`. Pass `columnVirtualizerOptions` to opt into column virtualization; omitting it renders every center column.

Column virtualization activates only when `tableLayout.width` is `"fixed"` and the table uses a single, ungrouped header row. Start- and end-pinned columns always render outside the virtual window. `scrollToRowIndex` targets the center row section, while `scrollToColumnIndex` targets the center visible leaf columns. Both controlled scrolling APIs share `scrollBehavior`, and `"auto"` alignment keeps an already-visible target in place.

```tsx
<DataGridTableVirtual
  columnVirtualizerOptions={{ enabled: true, overscan: 3 }}
...
| `estimateSize`             | `number`                                 | `48`     | Estimated row height in pixels for the row virtualizer.                                                                                                                                    |
| `overscan`                 | `number`                                 | `10`     | Number of rows to render outside the visible area.                                                                                                                                         |
| `columnVirtualizerOptions` | `DataGridTableColumnVirtualizerOptions`  | -        | Passing this prop opts into center-column virtualization for fixed-layout tables with ungrouped headers. Supports settings such as `enabled` and `overscan`; pinned columns always render. |
| `scrollBehavior`           | `ScrollBehavior`                         | `"auto"` | Scroll animation used when revealing a controlled row or column.                                                                                                                           |
| `scrollToColumnAlign`      | `"auto" \| "center" \| "start" \| "end"` | `"auto"` | Alignment used when revealing the target center column. `"auto"` only scrolls when the column is outside the visible area.                                                                 |
| `scrollToColumnIndex`      | `number`                                 | -        | Index within the center (non-pinned) visible leaf columns to reveal. Supports disabled column virtualization.                                                                              |
...
| `virtualizerOptions`       | `DataGridTableVirtualizerOptions<TData>` | -        | Optional passthrough for row virtualizer settings like `enabled`, `getItemKey`, `measureElement`, `rangeExtractor`, and `onChange`.                                                        |
...
| `data-column-index`     | `number`           | Index in the center visible leaf columns. Set on virtual table header and body cells for controlled column scrolling. |
