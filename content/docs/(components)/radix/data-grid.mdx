...
```bash
npm install radix-ui @tanstack/react-table class-variance-authority
```
...
`data-grid-table.tsx` exports the footer helpers and `DataGridTableRowPin`. `data-grid-table-virtual.tsx` adds virtualization, infinite scroll, and `footerContent` support. `data-grid-scroll-area.tsx` provides the dedicated radix scroll wrapper.
