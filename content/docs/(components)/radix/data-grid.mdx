...
| `i18n`                 | `DataGridI18nOverrides`   | -                      | Partial overrides for DataGrid-owned labels and accessibility text.    |
...
Pass component-scoped label overrides to the root `DataGrid`. Every key is optional and merges with the English defaults. Explicit message props on child components, such as `emptyMessage`, `loadingMessage`, `info`, or `rowsPerPageLabel`, take precedence over the root configuration.

```tsx
import type { DataGridI18nOverrides } from "@/components/reui/data-grid/data-grid-i18n"

const i18n: DataGridI18nOverrides = {
  labels: {
    sortAscending: "A-Z",
    sortDescending: "Z-A",
...
<DataGrid table={table} recordCount={data.length} i18n={i18n}>
  {/* ... */}
</DataGrid>
```

`DataGridI18nConfig` describes the fully resolved configuration. `DataGridI18nOverrides`, `DEFAULT_DATA_GRID_I18N`, and `mergeDataGridI18n()` are exported from `data-grid-i18n.ts`. Dynamic labels use typed callbacks: `unpinColumn(title)`, `selectedCount(count)`, and `paginationInfo({ from, to, count })`.

The `labels` section also covers sorting, column pinning and movement, column visibility, filtering, loading and empty states, infinite-scroll completion, pagination controls, row pinning and selection, and drag-reorder accessibility labels. Consumer-owned column titles, filter options, and cell values remain outside this contract.
...
The drag handle comes from `DataGridTableDndRowHandle` in a column of your own. It reads the row's sortable context, so it only works inside the rows this component renders; placed anywhere else it renders as a disabled grip. It also takes `className`, plus a `disabled` flag and an optional `disabledLabel` (falling back to `i18n.labels.reorderUnavailable`) for the cases where reordering is genuinely off, a sort being the usual one: the grip keeps its place in the gutter and reads as unavailable instead of vanishing and collapsing the column.
