---
title: Code Block
description: A shadcn code block with Shiki syntax highlighting, streaming, diffs, code folding and per-line interaction for AI chat and agent UIs.
component: true
base: radix
---
...
```bash
npx shadcn@latest add @reui/code-block
```
...
[Code Block components page](/components/code-block).
...
The same shape works without React Server Components. In Remix or TanStack
Start, return `lines` from a loader and pass them to the component.
...
Using react-markdown or Streamdown, `markdownCodeProps` reads the language and
the source out of the props given to `pre`:
...
1. **Defaults.** Blocks ship with `github-light` / `github-dark` and switch with
   your site theme automatically.
2. **Any shiki theme.** Register it in `codeBlockThemes` in the copied
...
3. **Design tokens.** Pass the built-in `css-variables` theme and the palette
...
The design-token example is the one embedded under
[Scrolling](#scrolling) above, and it also lives on the
[Code Block components page](/components/code-block).

### Dark Theme

Code often stays dark even on a light page. Wrap the block in a `dark` scope
and every semantic token inside follows; the primitive needs nothing.
...
the rest stay browsable on the [Code Block components page](/components/code-block).
...
[Code Block components page](/components/code-block): ANSI terminal output,
...
| Key                                          | Action                                                    |
| :------------------------------------------- | :-------------------------------------------------------- |
| <Kbd>Tab</Kbd>                               | Moves into the scroll region, which is a tab stop.        |
| <Kbd>↑</Kbd> <Kbd>↓</Kbd>                    | Moves the active line when `selectable`.                  |
| <Kbd>Shift</Kbd> + <Kbd>↑</Kbd> <Kbd>↓</Kbd> | Extends the selection.                                    |
| <Kbd>Enter</Kbd> <Kbd>Space</Kbd>            | Toggles the active line's selection in place.             |
| <Kbd>Home</Kbd> <Kbd>End</Kbd>               | Moves the active line to the first or last rendered line. |
| <Kbd>Esc</Kbd>                               | Clears the active line, when `selectable`.                |
| <Kbd>Enter</Kbd> <Kbd>Space</Kbd>            | Folds or unfolds the region when a fold toggle has focus. |
