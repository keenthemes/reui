...
All ReUI components are free and open-source - they install with this config as-is, with no license key or account required. Some installs also pull shared `@reui/*` primitives as dependencies; those supporting registry items are publicly accessible so the flow works without any extra setup. For more registry details, see the [Registry Guide](/docs/registry).

## Components

You can integrate ReUI components using the [**shadcn CLI**](https://ui.shadcn.com/docs/cli) for automation or by manual installation for full control.
Both methods support our entire collection of [Component Docs](/docs/) and [Components](/components).
