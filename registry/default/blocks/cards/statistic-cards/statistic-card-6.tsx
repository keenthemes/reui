...
import { CheckCircle, TrendingUp, TrendingDown, Pin, MoreHorizontal, Settings, TriangleAlert, Share2, Trash, BadgeAlertIcon } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center p-6 lg:p-8">
      ...
      <Card className="w-full md:w-96">
            ...
            <ul className="space-y-2">
              {activity.map((a, i) => (
                <li key={i} className="flex items-center justify-between gap-2.5 text-sm">
                  ...
                  <Badge variant={a.state === 'secondary' ? 'secondary' : 'destructive'} appearance="light" size="sm">{a.date}</Badge>
                </li>
              ))}
            </ul>
          </div>
        ...
        <CardFooter className="flex gap-2.5 h-auto py-3.5">
          ...
          <Button variant="primary" className="flex-1">Full Report</Button>
        </CardFooter>
      </Card>
    </div>
  );
